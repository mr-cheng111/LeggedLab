# -*- coding: utf-8 -*-
"""rsl_rl 5 兼容的 AMP-PPO。

第一版复用 rsl_rl PPO 的 actor/critic/storage，只在 step reward 和 update
中加入 WMP AMP 判别器损失。
"""
...
from legged_lab.amp import AMPDiscriminator, AMPLoader, AMPReplayBuffer, Normalizer
...
class WMPAMPPPO(PPO):
    ...
    def update(self):
        ...
        for batch, policy_batch, expert_batch in zip(generator, policy_generator, expert_generator):
            ...
            if self.desired_kl is not None and self.schedule == "adaptive":
                with torch.inference_mode():
                    kl_mean = torch.mean(self.actor.get_kl_divergence(batch.old_distribution_params, distribution_params))
            ...
            loss = (
                ...
                + amp_loss
                + self.grad_penalty_coef * grad_pen
            )
            ...
            mean_vel_predict_loss += float(vel_predict_loss.detach().cpu())
        ...
        loss_dict = {
            ...
            "vel_predict": mean_vel_predict_loss / num_updates,
